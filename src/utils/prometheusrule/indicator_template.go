...
var cpuUsagePercent = &models.Indicator{
	Name:  "cpu_usage",
	Templ: "%s(irate(container_cpu_usage_seconds_total{" + filter + "}[%s])) by (container_env_mesos_task_id) keep_common * 100 %s %s",
	Unit:  "%",
}
...
var tomcatThreadPool = &models.Indicator{
	...
	Templ: "%s(tomcat_threadpool_currentthreadcount{" + filter + "}) by (container_env_mesos_task_id) keep_common %s %s",
	Unit:  "个",
}
