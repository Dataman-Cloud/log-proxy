...
func (db *datastore) AckEvent(pk int, username string, groupname string) error {
	var result models.Event
	if db.Where("id = ?", pk).Where("user_name = ? OR group_name = ?", username, groupname).First(&result).RecordNotFound() {
...
func (db *datastore) ListUnackedEvent(page models.Page, username string, groupname string) map[string]interface{} {
	...
	db.Where("ack = ?", false).Where("user_name = ? OR group_name = ?", username, groupname).Find(&result).Count(&count)
